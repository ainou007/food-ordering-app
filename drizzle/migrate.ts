...
import { categories, orders, products_to_categories, products_to_orders, products, product_to_sizes, users, product_to_extras, sizes, extras } from "./schema";
...
async function main() {
  ...
  await db.insert(products_to_categories).values({ productId: Burger[0].id, categoryId: fastFoodCategory[0].id });

  console.log("✅ Products Asigned to the catigories successfully");

  // Create Sizes
  const small = await db.insert(sizes).values({ name: "Small", price: 3.99 }).returning({ id: sizes.id });
  const medium = await db.insert(sizes).values({ name: "Medium", price: 5.99 }).returning({ id: sizes.id });
  const large = await db.insert(sizes).values({ name: "Large", price: 7.99 }).returning({ id: sizes.id });
  ...
  // Create Extras
  const Fromage = await db.insert(extras).values({ name: "Fromage", price: 1.99 }).returning({ id: extras.id });
  const Frites = await db.insert(extras).values({ name: "Frites", price: 2.99 }).returning({ id: extras.id });
  const Poulet = await db.insert(extras).values({ name: "Poulet", price: 3.99 }).returning({ id: extras.id });
  const Dinde_fumee = await db.insert(extras).values({ name: "Dinde fumée", price: 4.99 }).returning({ id: extras.id });
